...
def decode_course_list(_text):
    ...
    for course in _raw_courses:
        if not course.select_one("a.not-open-tip") and not course.select_one("div.not-open-tip"):
            ...
            _course_detail["cover"] = course.select_one("img").attrs["src"]
...
def decode_course_point(_text):
    ...
    _course_point = {
        ...
        "points": [],
        "chapter": [],  # 用于存储章节标题
        "knowledgeCount": 0,  # 用于存储总知识点数目
        "knowledgeFinishCount": 0  # 用于存储已完成知识点数目
    }
    ...
    # 收集各章节标题
    _chapter_titles = _soup.find_all("a", class_="clicktitle")
    for _chapter_title in _chapter_titles:
        attts = _chapter_title.select_one("span").attrs
        if "title" in attts:
            _course_point["chapter"].append(attts["title"])

    # 收集总知识点和已完成知识点数目
    knowledgeCountText = _soup.select_one("h2.xs_head_name").get_text(strip=True)
    match = re.search(r'已完成任务点:\s*(\d+)/(\d+)', knowledgeCountText)
    if match:
        total_tasks = match.group(2)
        print(total_tasks)
        _course_point["knowledgeCount"] = int(total_tasks)
    _course_point["knowledgeFinishCount"] = int(
        _soup.select_one("h2.xs_head_name").select_one("span").get_text(strip=True))
